#!/usr/bin/env python3
"""
scenario_runner_las_vegas.py — thin entrypoint: Las Vegas default scenario + same CoT/plugin options as jtac_scenario_runner.

Uses jtac_edge/data/old_scenarios/las_vegas_scenario.json by default. Run from repo root: ``python archive/scenario_runner_las_vegas.py``. Override with --scenario if needed.

Usage:
    python archive/scenario_runner_las_vegas.py
    python scenario_runner_las_vegas.py --speed 2.0 --loop
    python scenario_runner_las_vegas.py --phone
    python scenario_runner_las_vegas.py --phone --once
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="JTAC_LOOP scenario runner — Las Vegas outskirts ISR demo (default scenario)"
    )
    p.add_argument(
        "--scenario",
        type=pathlib.Path,
        default=_EDGE / "data/old_scenarios/las_vegas_scenario.json",
        help="Path to scenario JSON (default: jtac_edge/data/old_scenarios/las_vegas_scenario.json)",
    )
...
def main():
    ...
    print(
        f"JTAC_LOOP scenario runner (Las Vegas)  |  {args.speed}x speed  |  frame step {args.frame_sec:g}s  |  "
