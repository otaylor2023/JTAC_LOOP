...
public class ExampleTest extends ATAKTestClass {
    private final JtacPluginRobot helloWorldRobot = new JtacPluginRobot();

    private static final String TAG = "ExampleTest";
    
    @BeforeClass
    public static void setupPlugin() throws Exception {
        JtacPluginRobot.installPlugin();
        JtacPluginRobot.sleep(1000);
        fixClassLoaderForClass(ExampleTest.class,
                "com.atakmap.android.jtac_plugin.plugin");

        JtacPluginRobot.sleep(1000);
        HelperFunctions.LoadedPlugin lp = HelperFunctions.getLoadedPlugin("com.atakmap.android.jtac_plugin.plugin");
        if (lp != null)
